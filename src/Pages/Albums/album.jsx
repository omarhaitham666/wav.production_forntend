...
//                                 <Link to={`/Artists/${album.artist_id}`} className='text-xl first-letter:uppercase hover:text-[#30B797] transition-all'>{album.artist.name}</Link>
//                             </div>
...
const Album = () => {
    const { loadSong, togglePlayPause, playing, currentSong, playSong } = useAudioPlayer();
    const { id } = useParams();
    const [album, setAlbum] = useState(null);
    ...
    useEffect(() => {
        axios.get(`http://127.0.0.1:8000/api/albums/${id}`)
            .then(response => {
                console.log("Album Response:", response.data);
                setAlbum(response.data.album); // تخزين بيانات الألبوم
                setSongs(response.data.songs); // تخزين الأغاني كمصفوفة مستقلة
                setLoading(false);
            })
            .catch(error => {
                setError("حدث خطأ أثناء تحميل البيانات.");
    ...
    if (loading) {
        return <p className="text-center py-10">جارٍ تحميل الألبوم...</p>;
    }
            ...
            <div className="py-12">
                <div className="container mx-auto">
                    <div className='flex flex-row gap-4'>
                        {album?.album_cover && (
                            <img
                                src={`http://127.0.0.1:8000/storage/${album.album_cover}`}
                        ...
                        <div className='flex flex-col justify-around'>
                            ...
                            <div className='flex flex-row gap-2 mt-4'>
                                <MdOutlineReplay className='text-3xl text-black hover:text-[#30B797] transition-all' />
                                <LuDownload
                                    className='text-3xl text-black hover:text-[#30B797] transition-all'
                                    onClick={() => downloadSong(album.link)}
                                />
                                <FaRegHeart
                                    className='text-3xl text-black hover:text-[#30B797] transition-all'
                    ...
                    {album && songs && songs.length > 0 ? (
                             songs.map((song, index) => (
                                <div key={song.id} className="flex flex-row gap-4 justify-between py-4 border-b border-[#DDDDDD]">
                                    <div className='flex flex-row gap-2 items-center'>
                                        ...
                                        <div className="flex flex-col">
                                            <h3 className="font-bold text-xl mb-1">{song.name}</h3>
                                            <Link to={`/Artists/${album.artist_id}`} className="text-sm text-gray-400 hover:text-[#30B797] transition-all">{song.artist}</Link>
                                        </div>
                                    </div>
                                    <div className="flex flex-row items-center gap-2">
                                        <MdOutlineReplay
                            ...
                            <p className="text-center py-5">لا توجد أغانٍ في هذا الألبوم.</p>
                        )} 
                    </div>
                </div>
            </div>
